...
def get_post_by_author(*, db_session: DbSession, author: User) -> Query[Post]:
    author_posts = db_session.query(Post).filter(Post.author_id == author.id)
    return author_posts


def get_mentions_by_post(*, db_session: DbSession, post: Post) -> Query[Post]:
...
def create_mention_for_post(*, db_session: DbSession, author: User, post: Post, context: PostPayload) -> Post:
...
def get_quotes_by_post(*, db_session: DbSession, post: Post) -> Query[Post]:
...
def create_quote_for_post(*, db_session: DbSession, author: User, post: Post, context: PostPayload) -> Post:
...
def update_post_for_user(*, db_session: DbSession, post: Post, context: PostPayload) -> Post:
    post.content = context.content
    db_session.commit()
    return post


def delete_post_for_user(*, db_session: DbSession, post: Post) -> None:
